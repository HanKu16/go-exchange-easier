...
import { useColorsContext } from "./SuggestionsColorsContext";
import type { SuggestionsColors } from "./SuggestionsColorsContext";

const FormInput = ( {label, id, value, isObligatory, 
  suggestions, onChange }: FormInputProps ) => {
  const suggestionsColors: SuggestionsColors  = useColorsContext()
  ...
  return (
      <div className="w-7/10 flex justify-center items-center">
        <div className="flex flex-col w-9/10 !pl-10">
          ...
          />
        {suggestions.map(s => (
          <p key={ s } className={`${suggestionsColors[id]} text-[0.8vw]`}>{`-${s}`}</p>
        ))}
        </div>
      </div>
  )
