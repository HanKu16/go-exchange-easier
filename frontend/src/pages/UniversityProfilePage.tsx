...
import { sendCreateReviewRequest } from '../utils/university'
...
const ShowReviewInputButton = (props: GoToSumbitReviewButtonProps) => {
  return (
    <Box sx={{display: 'flex', justifyContent: 'flex-start',
      width: '91%', margin: 'auto', marginBottom: 2}}>
      <Button variant='contained' startIcon={<AddIcon/>}
        onClick={props.handleClick}
        sx={{borderRadius: 2, textTransform: 'none', fontWeight: 'bold', 
          px: 3, py: 1, backgroundColor: '#04315f',
          '&:hover': {
...
const ReviewInput = (props: ReviewInput) => {
    ...
    const isSubmitDisabled = !postText.trim() || ratingValue === null || ratingValue === 0
    const { showAlert } = useSnackbar()
    ...
    return (
      <Box sx={{width: '91%', margin: 'auto', p: 3, boxShadow: 3, 
        borderRadius: 2, background: 'white', marginBottom: 4}}>
        <Box sx={{display: 'flex', justifyContent: 'space-between'}}>
          <Typography variant='h5' gutterBottom>
            Submit your review
          </Typography>
          <Button variant='contained' onClick={props.handleGoBackClick}
            sx={{borderRadius: 2, textTransform: 'none', fontWeight: 'bold', 
              px: 3, py: 1, backgroundColor: '#04315f',
              '&:hover': {
                backgroundColor: '#064080'
        ...
        <Stack direction='row' spacing={2} alignItems='center' sx={{my: 2}}>
          ...
          <Rating name='post-rating' value={ratingValue} precision={1}
        ...
        <TextField label='Text' placeholder='What do you think?' multiline
          rows={4} fullWidth variant='outlined' value={postText}
          onChange={(e) => setPostText(e.target.value)} sx={{mb: 3}}/>
    ...
    <>
      {isLgScreen ? (
        <Container sx={{ backgroundColor: '#182c44', minHeight: '100vh', display: 'flex', 
          ...
          <img alt='Country flag' src={`/flags/${props.university.countryName}.png`} 
          ...
          <Container sx={{width: '100%', paddingTop: 3}}>
            ...
            {props.university.linkToWebsite && 
              <Box sx={{display: 'flex', marginTop: 1, marginLeft: 0.5}}>
                ...
                <Link href={props.university.linkToWebsite} underline="hover"
            ...
            <FollowButton universityId={props.university.id} isFollowed={isFollowed}
...
const FeedPanel = (props: FeedPanelProps) => {
  ...
  const getReviews = async (pageNumber: number) => {
    const result = await sendGetUniversityReviewsRequest(
      props.universityId, pageNumber, pageSize)
    if (result.isSuccess) {
  ...
  return (
    <Box sx={{paddingBottom: 4, display: 'flex', flexDirection: 'column', 
      ...
      <Box>
        ...
        {showReviewInput ?
          <ReviewInput handleGoBackClick={handleShowingSubmitReviewSection} 
            handleSuccessfulCreation={(createdReviewProps: UniversityReviewProps) => 
              setReviewsProps([createdReviewProps, ...reviewProps])}
              universityId={props.universityId}/> :
          <ShowReviewInputButton handleClick={handleShowingSubmitReviewSection}/>
        }
        {getReviewsContent()}
      </Box>
      {(totalPagesCount > 1) &&
...
type UniversityProfileFetchStatus = 
  DataFetchStatus |
  'universityNotFound'
...
const UniversityProfilePage = () => {
  ...
  if (universityId === undefined || !isInteger(universityId)) {
    return <NotFoundPage icon={SearchOffIcon} title='University not found'
  ...
  if (universityProfileFetchStatus === 'loading') {
    ...
    return <NotFoundPage icon={SearchOffIcon} title='University not found'
      subheader='Profile you are looking for was deleted or does not exist'/>  
  } else if ((universityProfileFetchStatus === 'success') && 
