    ...
    <>
      {isLgScreen ? (
        ...
        >
          ...
          <Box
            sx={{ width: "100%", paddingX: { lg: 5 }, paddingTop: { lg: 4 } }}
          >
        ...
        >
          ...
          <Box sx={{ paddingX: 3, paddingTop: 4 }}>
            {props.description.trim() !== "" ? (
              <>
                <Typography
                  sx={{ color: "white", fontWeight: "600", paddingBottom: 1 }}
                >
                  About
                </Typography>
                <Typography
...
const FeedPanel = (props: FeedPanelProps) => {
  ...
  const getExchangesContent = () => {
    if (exchangesFetchStatus === "success") {
      if (
        exchangesProps !== undefined &&
        exchangesProps.exchanges.length !== 0
      ) {
        return (
          <>
          ...
          <>
            {props.isOwnProfile ? (
              ...
              <NoContent
                ...
                subheader="This user hasn't add any exchange."
              />
            )}
          </>
        );
      ...
      return (
        <ContentLoadError
          title="Connection error"
          subheader="An error occurred while fetching exchanges."
        />
      );
...
type UserProfileFetchStatus = DataFetchStatus | "userNotFound";
    ...
    if (result.isSuccess) {
      ...
      const universityName = data.homeUniversity
        ? data.homeUniversity.englishName
          ? data.homeUniversity.englishName
          : data.homeUniversity.nativeName
