...
public class UniversitySpecification {
    ...
    public static Specification<University> fromFilter(UniversityFilters filter) {
        ...
        spec = SpecificationUtils.append(spec, filter.englishName(), UniversitySpecification::hasEnglishName);
        spec = SpecificationUtils.append(spec, filter.nativeName(), UniversitySpecification::hasOriginalName);
        spec = SpecificationUtils.append(spec, filter.cityId(), UniversitySpecification::hasCityId);
        spec = SpecificationUtils.append(spec, filter.countryId(), UniversitySpecification::hasCountryId);
        return spec;
    }
    ...
    public static Specification<University> hasOriginalName(
        ...
        return (root, query, cb) -> {
            ...
            return cb.like(cb.lower(root.get("originalName")), searchPattern);
        };
    }
    ...
    public static Specification<University> hasEnglishName(
        ...
        return (root, query, cb) -> {
            ...
            return cb.like(cb.lower(root.get("englishName")), searchPattern);
        };
    }

    public static Specification<University> hasCityId(int cityId) {
        return (root, query, cb) -> {
            Join<University, City> citiesJoin = root.join("city");
