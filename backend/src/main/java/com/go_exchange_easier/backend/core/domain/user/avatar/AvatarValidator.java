...
public class AvatarValidator implements ConstraintValidator<ValidAvatar, MultipartFile> {
    ...
    public boolean isValid(
        ...
        if (contentType == null || !contentType.equals("image/webp")) {
            context.buildConstraintViolationWithTemplate("Invalid MIME (expected was image/webp).")
                    .addConstraintViolation();
            return false;
        }
        try (InputStream inputStream = file.getInputStream()) {
            ...
            if (inputStream.read(header) < 12) {
                context.buildConstraintViolationWithTemplate("File is broken or too short.")
                        .addConstraintViolation();
                return false;
            }
            ...
            if (!isRiff || !isWebP) {
                context.buildConstraintViolationWithTemplate("File content does not match extension .webp.")
                        .addConstraintViolation();
                return false;
            }
        } catch (IOException e) {
