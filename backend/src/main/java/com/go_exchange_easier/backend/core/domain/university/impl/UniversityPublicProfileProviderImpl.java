...
public class UniversityPublicProfileProviderImpl implements UniversityPublicProfileProvider {

    private final UniversityRepository universityRepository;
    private final CountryService countryService;

    @Override
    @Cacheable(value = "university-public-profiles", key = "'university:' + #universityId")
    public UniversityPublicProfile getProfile(int universityId) {
        List<Object[]> rows = universityRepository.findProfileById(universityId);
        if (rows.isEmpty()) {
        ...
        return new UniversityPublicProfile(
                id, originalName, englishName, linkToWebsite, new CityDetails(
                cityId,
                cityName,
                new CountryDetails(
                        countryId,
                        countryName,
                        countryFlagKey != null ? countryService.getFlagUrl(countryFlagKey) : null
                )
        )
        );
    }
