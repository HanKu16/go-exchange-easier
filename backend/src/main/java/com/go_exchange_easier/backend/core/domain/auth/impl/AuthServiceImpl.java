...
public class AuthServiceImpl implements AuthService {
    ...
    public TokenBundle login(
        ...
        if (authentication.getPrincipal() instanceof AuthenticatedUser authenticatedUser) {
            String accessToken = jwtTokenGenerator.generateAccessToken(
                    ...
                    authenticatedUser.getNick(),
                    authenticatedUser.getAvatarKey(),
    ...
    @Override
    @Transactional
    public TokenBundle refresh(
            String refreshToken,
            HttpServletRequest servletRequest
    ) {
        RefreshToken oldToken = refreshTokenRepository.findByHashedToken(getHashedToken(refreshToken))
                .orElseThrow(() -> new TokenNotFoundException("Token " + refreshToken + " was not found."));
        if (oldToken.isRevoked()) {
            throw new TokenRevokedException(
                    "There was attempt of usage token that is revoked. Probably someone stole the token.");
        }
        ...
        Principal principal = principalRepository.findById(userId)
    ...
    public TokenBundle logout(String refreshToken) {
        ...
        if (refreshToken != null) {
            refreshTokenRepository.revokeByHashedToken(getHashedToken(refreshToken));
        }
    ...
    private void validateDeviceMatch(
            RefreshToken oldToken,
            HttpServletRequest request
    ) {
        String headerDeviceId = request.getHeader("X-Device-Id");
        String tokenDeviceId = (oldToken.getDeviceId() != null) ? oldToken.getDeviceId()
                .toString() : null;
        if ((tokenDeviceId != null) && !tokenDeviceId.equals(headerDeviceId)) {
            throw new DeviceMismatchException(
                    "Token bound to device " + tokenDeviceId + " but request came from " + headerDeviceId);
        }
