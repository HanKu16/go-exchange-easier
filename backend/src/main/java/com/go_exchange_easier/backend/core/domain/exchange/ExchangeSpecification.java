...
import com.go_exchange_easier.backend.core.common.jpa.SpecificationUtils;
...
public class ExchangeSpecification {
    ...
    public static Specification<Exchange> fromFilter(ExchangeFilters filter) {
        ...
        spec = SpecificationUtils.append(spec, filter.universityId(), ExchangeSpecification::hasUniversityId);
        spec = SpecificationUtils.append(spec, filter.countryId(), ExchangeSpecification::hasCountryId);
        spec = SpecificationUtils.append(spec, filter.cityId(), ExchangeSpecification::hasCityId);
        spec = SpecificationUtils.append(spec, filter.fieldOfStudyId(), ExchangeSpecification::hasFieldOfStudyId);
        spec = SpecificationUtils.append(spec, filter.startDate(), ExchangeSpecification::hasStartDateAtLeast);
        spec = SpecificationUtils.append(spec, filter.endDate(), ExchangeSpecification::hasEndDateAtMost);
        spec = SpecificationUtils.append(spec, filter.userId(), ExchangeSpecification::hasUserId);
        return spec;
    }

    public static Specification<Exchange> hasUniversityId(short universityId) {
        return (root, query, cb) -> {
            Join<Exchange, University> universitiesJoin = root.join("university");
            return cb.equal(universitiesJoin.get("id"), universityId);
        };
    }
    ...
    public static Specification<Exchange> hasFieldOfStudyId(short fieldOfStudyId) {
        return (root, query, cb) -> {
            Join<Exchange, FieldOfStudy> fieldsOfStudy = root.join("fieldOfStudy");
